...
Axolotl-PM plugins are required to declare which API versions they are compatible with. This is used to decide whether or not to load a plugin, and to gracefully degrade when the plugin is not compatible with the given server version.

As of Axolotl-PM 3.0.0, the API version is the same as the server version. This version is a semantic `major.minor.patch` version number. Read more about [semantic versioning](https://semver.org/).
...
For example, if the feature you want was first added in `3.1.0`, you can require `3.1.0` as a minimum even if the currently supported version is `3.2.0`. This is to ensure that users get a smoother experience no matter what version of Axolotl-PM they are using.
